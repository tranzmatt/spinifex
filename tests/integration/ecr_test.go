...
// Matching the E2E source's naming keeps a future side-by-side diff greppable.
...
func TestECRControlPlane(t *testing.T) {
	...
	t.Run("repository policy round-trips", func(t *testing.T) {
		...
		_, err := c.SetRepositoryPolicy(&ecr.SetRepositoryPolicyInput{
			RepositoryName: aws.String(repo),
			PolicyText:     aws.String(policy),
		})
		require.NoError(t, err)

		got, err := c.GetRepositoryPolicy(&ecr.GetRepositoryPolicyInput{
			RepositoryName: aws.String(repo),
		})
		require.NoError(t, err)
	...
	t.Run("lifecycle policy round-trips", func(t *testing.T) {
		...
		_, err := c.PutLifecyclePolicy(&ecr.PutLifecyclePolicyInput{
			RepositoryName:      aws.String(repo),
			LifecyclePolicyText: aws.String(policy),
		})
		require.NoError(t, err)

		got, err := c.GetLifecyclePolicy(&ecr.GetLifecyclePolicyInput{
			RepositoryName: aws.String(repo),
		})
		require.NoError(t, err)
	...
	t.Run("DeleteRepository removes it", func(t *testing.T) {
		_, err := c.DeleteRepository(&ecr.DeleteRepositoryInput{
			RepositoryName: aws.String(repo),
			Force:          aws.Bool(true),
		})
		require.NoError(t, err)

		_, err = c.DescribeRepositories(&ecr.DescribeRepositoriesInput{
			RepositoryNames: []*string{aws.String(repo)},
		})
...
// AWS:<jwt> credential — the input to docker login. Minting needs only
// ECRTokenIssuer, wired unconditionally by StartGateway, so no daemon-lite
// subscription is required here.
func TestECRGetLoginPassword(t *testing.T) {
	gw := StartGateway(t)
	pass := ecrGetLoginPassword(t, gw.ECRClient(t))
