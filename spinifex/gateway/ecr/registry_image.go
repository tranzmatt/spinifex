...
func (reg *Registry) StoreManifest(ctx context.Context, account, repo, ref, contentType string, body []byte) (string, error) {
	scoped := reg.forAccount(account)
	if err := scoped.requireRepo(ctx, repo); err != nil {
		return "", err
	}
	...
	if ref != "" && !ecr.ValidateDigest(ref) {
		meta, err := scoped.Meta.GetRepo(ctx, account, repo)
		if err != nil {
			return "", err
		}
		if meta.TagMutability() == ecr.TagMutabilityImmutable {
			existing, err := scoped.Meta.GetTag(ctx, account, repo, ref)
			switch {
	...
	children, code, vErr := scoped.validateManifest(ctx, repo, contentType, body)
	if vErr != nil {
		return "", &ManifestStoreError{http.StatusBadRequest, code, vErr.Error()}
	}

	if _, err := scoped.Store.PutObject(ctx, &s3.PutObjectInput{
	...
	if err := scoped.Meta.PutManifestMeta(ctx, account, repo, ecr.ManifestMeta{
		Digest:       digest,
	...
	if ref != "" && !ecr.ValidateDigest(ref) {
		if err := scoped.Meta.PutTag(ctx, account, repo, ref, digest); err != nil {
			return "", err
		}
	}
	return digest, nil
...
func (reg *Registry) GetManifest(ctx context.Context, account, repo, ref string, acceptedTypes []string) ([]byte, string, string, error) {
	scoped := reg.forAccount(account)
	digest, ok := scoped.resolveManifestDigest(ctx, repo, ref)
	if !ok {
		return nil, "", "", ErrImageNotFound
	}
	meta, err := scoped.Meta.GetManifestMeta(ctx, account, repo, digest)
	if err != nil {
	...
	out, err := scoped.Store.GetObject(ctx, &s3.GetObjectInput{
		Bucket: aws.String(scoped.bucket()),
		Key:    aws.String(ecr.ManifestKey(repo, digest)),
	})
	if err != nil {
...
func (reg *Registry) ListImages(ctx context.Context, account, repo string) ([]ImageRecord, error) {
	scoped := reg.forAccount(account)
	if _, err := scoped.Meta.GetRepo(ctx, account, repo); err != nil {
		return nil, err
	}

	tags, err := scoped.Meta.ListTags(ctx, account, repo)
	if err != nil {
		return nil, err
	}
	tagsByDigest := make(map[string][]string)
	for _, tag := range tags {
		digest, err := scoped.Meta.GetTag(ctx, account, repo, tag)
		if err != nil {
			if errors.Is(err, ecr.ErrNotFound) {
				continue
			}
	...
	keys, err := scoped.Meta.ListManifests(ctx, account, repo)
	if err != nil {
		return nil, err
	}
	records := make([]ImageRecord, 0, len(keys))
	for _, key := range keys {
		meta, err := scoped.Meta.GetManifestMeta(ctx, account, repo, key)
		if err != nil {
			if errors.Is(err, ecr.ErrNotFound) {
				continue
			}
...
func (reg *Registry) DeleteImage(ctx context.Context, account, repo, tag, digest string) (string, error) {
	...
	if tag != "" && digest == "" {
		resolved, err := scoped.Meta.GetTag(ctx, account, repo, tag)
		if err != nil {
			if errors.Is(err, ecr.ErrNotFound) {
		...
		if err := scoped.Meta.DeleteTag(ctx, account, repo, tag); err != nil {
			return "", err
		}
		return resolved, nil
	}
	...
	for _, t := range tags {
		d, err := scoped.Meta.GetTag(ctx, account, repo, t)
		if err != nil {
			if errors.Is(err, ecr.ErrNotFound) {
				continue
			}
		...
		if d == digest {
			if err := scoped.Meta.DeleteTag(ctx, account, repo, t); err != nil {
				return "", err
			}
		}
	}
	if err := scoped.Meta.DeleteManifestMeta(ctx, account, repo, digest); err != nil {
		return "", err
	}
	...
	scoped.reclaimManifest(ctx, account, repo, digest, meta.ChildDigests)
	return digest, nil
}
...
func (reg *Registry) reclaimManifest(ctx context.Context, account, repo, digest string, children []string) {
	if _, err := reg.Store.DeleteObject(ctx, &s3.DeleteObjectInput{
		Bucket: aws.String(ecr.AccountBucket(account)),
		Key:    aws.String(ecr.ManifestKey(repo, digest)),
	}); err != nil {
		slog.WarnContext(ctx, "ECR/GC: manifest object delete failed", "repo", repo, "digest", digest, "err", err)
	}

	if len(children) == 0 {
		return
	}
	referenced, err := reg.referencedDigests(ctx, account)
	if err != nil {
		slog.WarnContext(ctx, "ECR/GC: reference scan failed, skipping blob reclaim", "account", account, "err", err)
		return
	}
	for _, child := range children {
		...
		if _, err := reg.Store.DeleteObject(ctx, &s3.DeleteObjectInput{
			...
			slog.WarnContext(ctx, "ECR/GC: blob delete failed", "digest", child, "err", err)
		}
	}
}
...
func (reg *Registry) referencedDigests(ctx context.Context, account string) (map[string]bool, error) {
	repos, err := reg.Meta.ListRepos(ctx, account)
	if err != nil {
		return nil, err
	}
	referenced := make(map[string]bool)
	for _, repo := range repos {
		digests, err := reg.Meta.ListManifests(ctx, account, repo)
		if err != nil {
			return nil, err
		}
		for _, key := range digests {
			meta, err := reg.Meta.GetManifestMeta(ctx, account, repo, key)
			if err != nil {
				if errors.Is(err, ecr.ErrNotFound) {
					continue
				}
