...
func (s *Service) EnforceVolumeModify(ctx context.Context, natsConn *nats.Conn, accountID, volumeID string, newGiB int) error {
	if s.Exempt(accountID) {
		return nil
	}
	total, oldGiB, err := s.volumeUsage(ctx, natsConn, accountID, volumeID)
	if err != nil {
		return err
	}
