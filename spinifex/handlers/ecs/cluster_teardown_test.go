...
func TestService_DeleteCluster_CascadesAndSweeps(t *testing.T) {
	...
	out, err := svc.DeleteCluster(&ecs.DeleteClusterInput{Cluster: aws.String("web")}, testAccountID)
	require.NoError(t, err)
...
func TestService_DeleteCluster_Unknown(t *testing.T) {
	...
	_, err := svc.DeleteCluster(&ecs.DeleteClusterInput{Cluster: aws.String("ghost")}, testAccountID)
	require.Error(t, err)
}
...
func TestService_DeregisterTaskDefinition_InactivatesRevision(t *testing.T) {
	...
	out, err := svc.DeregisterTaskDefinition(&ecs.DeregisterTaskDefinitionInput{
	...
	d, err := svc.DescribeTaskDefinition(&ecs.DescribeTaskDefinitionInput{TaskDefinition: aws.String("app:1")}, testAccountID)
	require.NoError(t, err)
...
func TestService_DeregisterTaskDefinition_RequiresRevision(t *testing.T) {
	...
	_, err := svc.DeregisterTaskDefinition(&ecs.DeregisterTaskDefinitionInput{TaskDefinition: aws.String("app")}, testAccountID)
	require.Error(t, err)
}
...
func TestService_DeregisterContainerInstance_ForceStopsAndRemoves(t *testing.T) {
	...
	_, err = svc.DeregisterContainerInstance(&ecs.DeregisterContainerInstanceInput{
		Cluster: aws.String("web"), ContainerInstance: aws.String("i-1"),
	}, testAccountID)
	require.Error(t, err)

	// With force, the task is stopped and the instance record removed.
	out, err := svc.DeregisterContainerInstance(&ecs.DeregisterContainerInstanceInput{
...
func TestService_UpdateContainerInstancesState_DrainsServiceTasks(t *testing.T) {
	...
	out, err := svc.UpdateContainerInstancesState(&ecs.UpdateContainerInstancesStateInput{
