...
const IMPORT_COMMAND = "spx admin images import --name spinifex-ecs-node"

export function EcsSystemImageRequired({
  isRechecking,
  onRecheck,
}: {
  ...
  return (
    <div className="mb-6 max-w-2xl rounded-md border border-tactical-amber/40 bg-tactical-amber/5 p-6">
      <div className="flex items-start gap-3">
        <TriangleAlert className="mt-0.5 size-5 shrink-0 text-tactical-amber" />
        <div className="space-y-4">
          <div className="space-y-1">
            <h2 className="text-sm font-semibold text-foreground">
              ECS system image not found
            </h2>
            <p className="text-xs text-muted-foreground">
              ECS clusters need a Spinifex-managed node image (containerd +
              ecs-agent) to launch container instances. Import it before
              creating a cluster.
            </p>
          </div>
