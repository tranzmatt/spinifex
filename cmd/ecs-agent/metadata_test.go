...
func TestFetchInstanceMetadata_OK(t *testing.T) {
	...
	got, err := fetchInstanceMetadata(srv.Client(), srv.URL+"/latest")
	if err != nil {
...
func TestFetchInstanceMetadata_TokenRequired(t *testing.T) {
	...
	srv := metadataStub(t, true)
	if _, err := fetchInstanceMetadata(srv.Client(), srv.URL+"/latest"); err != nil {
...
func TestFetchInstanceMetadata_BadDocument(t *testing.T) {
	...
	mux.HandleFunc("/latest/meta-data/instance-id", func(w http.ResponseWriter, r *http.Request) { _, _ = w.Write([]byte("i-1")) })
	mux.HandleFunc("/latest/meta-data/placement/availability-zone", func(w http.ResponseWriter, r *http.Request) { _, _ = w.Write([]byte("z")) })
	mux.HandleFunc("/latest/dynamic/instance-identity/document", func(w http.ResponseWriter, r *http.Request) { _, _ = w.Write([]byte("not json")) })
	srv := httptest.NewServer(mux)
	t.Cleanup(srv.Close)
	if _, err := fetchInstanceMetadata(srv.Client(), srv.URL+"/latest"); err == nil {
